...
describe ChangeAgentService do
  ...
  it "returns the view for registration succeeded when the form is correctly submitted" do
    ...
    ChangeAgentService.handle({:name => 'John Denver', :email => 'john@denver.com'}).should == :'change_agents/success'
  end
  ...
  it "returns the view for the join page when the parameters are invalid" do
    ...
    ChangeAgentService.handle({:name => "James Earl Jones", :email=>"darth_mominous@orgspot"}).should == :'change_agents/join'
